...
const DesignBoard = () => {
    ...
    useEffect(() => {
        if (delteKeyPressed === true) {
            let data = itemList.filter((item, index) => index != currentItemIndex - 1);
            setItemList([...data]);
        }
    }, [delteKeyPressed]);
    ...
    const parsePositionNumber = (parseValue, OffsetValue) => {
        ...
        if (part == "%")
            return parseFloat(parseValue) * OffsetValue / 100;
        else
            return parseFloat(parseValue);
    }

    const updatePosition = (selectedIndex, updatedPosition) => {
        if (parseFloat(updatedPosition.left) * boardRef.current.offsetWidth / 100 < 20) {
            updatedPosition.left = 20 * 100 / boardRef.current.offsetWidth + "%";
        }
        if (parseFloat(updatedPosition.top) * boardRef.current.offsetHeight / 100 < 20) {
            ...
            <div className="design-pane" >
                {
                    itemList.map((item, index) => {
                        return (
                            ...
                            >
                                <img src={item.imgUrl} draggable={false} style={{ width: '100%', height: '100%' }} />
                            </PositionableContainer>
                        )
                    })
                }
